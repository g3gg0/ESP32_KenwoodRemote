...
void mqtt_raw_code_received(const t_ha_entity *entity, void *ctx, const char *message)
{
    int cmd = 0;
    char msg[128];
    ...
    {
        mqtt_publish_string("feeds/string/%s/error", "Start all");
        for (int cmd = 0x0000; cmd < 0x10000; cmd += 0x40)
        {
            snprintf(msg, sizeof(msg), "Try %04X", cmd);
            mqtt_publish_string("feeds/string/%s/error", msg);
            ken_try(cmd, 0x10);
        }

        mqtt_publish_string("feeds/string/%s/error", "Finished all");
        return;
    }

    int ret = sscanf(message, "%x", &cmd);
    if (ret != 1)
    {
    ...
    snprintf(msg, sizeof(msg), "Send command 0x%04X", cmd);
    mqtt_publish_string("feeds/string/%s/error", msg);
...
void mqtt_setup()
{
    ...
    memset(&entity, 0x00, sizeof(entity));
    entity.id = "command";
    entity.name = "Raw command";
    entity.type = ha_text;
    entity.cmd_t = "command/%s/raw_code";
    entity.received = &mqtt_raw_code_received;
    ha_add(&entity);
}
